(function ($) {
  ...
  $(".mini_calendar").find("tbody td").filter(function () {
    return $(this).text().trim() === (today.getDate() + 1).toString() &&
      today.getMonth() + 1 === 3 &&
      today.getFullYear() === 2026
   }).html(function () {
    return `<div id="today">${today.getDate() + 1}</div>`;
  })
  ...
  $(document).on("click", "#donate-modal .crypto-item", function () {
    ...
    parent
      .find(".cryptos-box-view .coin-address")
      .html($(this).data("address"));
    parent
      .find(".cryptos-box-view .coin-qr-code")
      .html("")
      .qrcode({ width: 160, height: 160, text: $(this).data("address") });
  });
  ...
  if ($("#wish-form").length) {
    $("#wish-form").validate({
      ...
      messages: {
        name: {
          required:
            typeof messageNameRequired != "undefined"
              ? messageNameRequired
              : "Vui lòng nhập tên của bạn.",
          minlength:
            typeof messageNameMinlength != "undefined"
              ? messageNameMinlength
              : "Tên phải lớn hơn 3 ký tự.",
        },
        content: {
          required:
            typeof messageContentRequired != "undefined"
              ? messageContentRequired
              : "Vui lòng nhập lời chúc.",
          minlength:
            typeof messageContentMinlength != "undefined"
              ? messageContentMinlength
              : "Lời chúc phải lớn hơn 10 ký tự.",
        },
        email: {
          email:
            typeof messageEmailEmail != "undefined"
              ? messageEmailEmail
        ...
        $.ajax({
          ...
          success: function (res) {
            ...
            if (!res.error) {
              ...
              $(".wish-box").prepend(
                '<div class="wish-box-item bg"><strong>' +
                  $(form)
